...
//서브 함수 선언 부분
//***********************
int  draw_title(int x, int y);//         타이틀
int draw_maker();//					     소개
void draw_loading_screen();//            로딩
int draw_select_song(int x_m, int y_m);//곡 선택 부분
void draw_equalizer(int *equalizer_count,char pic[][8]);//이퀄라이져
void draw_song(int *sel,char key,struct song *);//곡 목록 그리기
void draw_setting();//			     설정화면
void auto_sync();//                  자동 싱크 모듈
void draw_sync_check_note(int vac[][11]);//자동 싱크 모듈 테스트 함수
void wirte_somnail(struct song *);//     곡정보 작성
void move_draw_song(char key,struct song *);//곡 목록 옮기기
int draw_game_screen(int note_set,int spd,int song,int diff);//게임 화면
void draw_key_effect(char *key);//       키를 누를 시 기어의 반응
void delete_key_FX(char *key);//         기어 반응 후 초기화
void draw_note(int set,int vac[4][24]); //노트
void note_to_note(int *num, int notelen, char f_note[][5], int note[4][24]);//텍스트로 노트 불러와서 게임으로 옮기기
void draw_offset();//					 음악, 노트 싱크 조절
int judge_acc(char *key, int note[4][24]);//정확도 판단
void draw_judge(int *jud);//			 정확도 이펙트.
void erease_judge();//					 이펙트 삭제
void draw_score_and_combo();//           점수, 콤보
int stopmenu();//						 정지메뉴
int draw_result();//                     결과화면
...
int draw_maker()
{
	...
	for (;;)
	{
		...
		system("cls");
		system("cls");
		printf("제작자 : 심이진 ( Sim Lee Jin )\n");
		printf("학번 : 2109\n");
		printf("게임 설명 : 내려오는 노트들을 리듬에 따라 맞추는 게임입니다.\n");
		printf("\n조작키 : D F     J K\n\n");
		printf("\n\n\n\n돌아가기 : space");
...
int draw_select_song(int x_m, int y_m)
{
	...
	char pic[8][8] = { 0, };
	int speed = 50, song = 1, difficulty = 1;//속도, 음악,난이도
